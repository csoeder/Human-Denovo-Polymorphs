...
#	bsub -J mapsplut_$FOLDER -o mapsplut.lsf.out -M 75 -q week python /nas02/apps/mapsplice-2.1.4/src/MapSplice-v2.1.4/mapsplice.py -c /netscr/csoeder/1kGen/data/hg19_split/ -x /netscr/csoeder/1kGen/data/hg19_bowtieindex/hg19 -1 ERR*_1.fastq -2 ERR*_2.fastq
	...
	bsub -J splutDex_$FOLDER -o splutDex.lsf.out samtools index "$FOLDER"_mapsplice_alignment.sort.bam
	bsub -J bam2bed_$FOLDER -o bam2bed.lsf.out "bedtools bamtobed -bed12 -i "$FOLDER"_mapsplice_alignment.sort.bam > "$FOLDER"_mapsplice_alignment.bed"
fi
#########################################
###Assemble the reads####################
