...
if [ ! -f $FOLDER_mapsplice_alignment.sam ]; then
	...
	bsub -J mvsplut_$FOLDER -w "done(mapsplut_$FOLDER)" -o mvsplut.lsf.out  mv mapsplice_out/alignments.sam $FOLDER_mapsplice_alignment.sam
	bsub -J splutvert_$FOLDER -w "done(mvsplut_$FOLDER)" -o splutvert.lsf.out "samtools view -Sbh $FOLDER_mapsplice_alignment.sam > $FOLDER_mapsplice_alignment.bam"
	bsub -J bamsort_$FOLDER -w "done(splutvert_$FOLDER)" -o bamsort.lsf.out samtools sort $FOLDER_mapsplice_alignment.bam $FOLDER_mapsplice_alignment.sort
	bsub -J splutDex_$FOLDER -w "done(bamsort_$FOLDER)" -o splutDex.lsf.out samtools index $FOLDER_mapsplice_alignment.sort.bam
	bsub -J bam2bed_$FOLDER -w "done(bamsort_$FOLDER)" -o bam2bed.lsf.out "bedtools bamtobed -bed12 -i $FOLDER_mapsplice_alignment.sort.bam > $FOLDER_mapsplice_alignment.bed"
