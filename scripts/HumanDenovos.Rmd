...
data_sets.df.sparse <- data_sets.df %>% filter(subgroups=='all') %>%  select(c(name,sex,population,readLen,phase1Geno))
...
softwareUsed.smol.df %>% mutate(bibtex = paste("@", bibtex, sep="")) %>% gt() %>% fmt_markdown(columns = c("bibtex"))
```
...
The mapped transcripts were given a first-pass filter, in which at least 75 reads and at least 75% of all bases must match for the alignment to be retained. 
...
RNA-Seq reads mapping to the same region as an anomalous transcript were collected; then mapped reads within a read’s length (75 bp) of the collection were iteratively added to it. If the collection of accumulated reads indirectly associated with the transcript ever overlapped a forbidden region, the transcript was flagged.  
...
Out of an initial ___ transcripts (___ per person), a total of ___ were retained as having no evidence of transcriptional association with any annotated gene or complex repetitive element. These were then scanned for open reading frames. 

 The remaining transcripts were considered to be 'anomalous'.

### ORFome
 

#### Extraction
ORFs were extracted from the anomalous transcripts; because the 1000 Genomes RNA-Seq used an unstranded protocol, the anomalous transcripts were reverse complemented and ORFs extracted.
#### Filtration
These were size-filtered with a lower length cutoff of 75 bp (see Hashimoto 2001 in Ruiz-Orera 2014) The size-filtered candidates were BLATed against the human genome and those with more than one quality hit were flagged as having genomic duplicates. 
...
ORFs were also BLATed against the chimp and gorilla genomes to establish possible sites of homology. These genomic locations were then given a first-order scan for coding sequence by enumerating all the same-strand ORFs in the site and the 4kb surrounding it. If one of the ORFs overlapped 75% or more the homologous site, it was flagged as having a homologous coding sequence in the primate in question. (h/t Jeremy Wang)
