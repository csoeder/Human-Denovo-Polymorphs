...
knitr::opts_knit$set(root.dir = '/Users/csoeder/Research/Human-Denovo-Polymorphs')
...
# fig_cnt <- fig_cnt + 1
# thing.gg <- asdfasdfasd + labs(title = paste("Figure ",fig_cnt, ". Percent of Reads with a mean QUAL > 30", sep = ""), y="Percent QUAL > 30", x="") + theme_bw() 
# thing.gg
...
refGenomes_detail <- read_delim("utils/hg38.fa.fai", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
...
ORFs were also BLATed against the reference genomes of four other closely related primates (chimpanzee panTro6, gorilla gorGor5, orangutan ponAbe2, gibbon nomLeu3) to establish possible sites of homology. These genomic locations were then given a first-order scan for coding sequence by enumerating all the same-strand ORFs in the site and the 10kb surrounding it. If one of these ORFs overlapped 75% or more the homologous site, it was flagged as having a homologous coding sequence in the primate in question. (h/t Jeremy Wang) Although we will later look at the definitive orthologies determined through syntenic multiple sequence alignment, this approach is used because it is more aggressive in eliminating candidates since it can only increase the number of sites examined, and thus the opportunities for an ORF to be excluded (consider a situation in which an ORF has been duplicated in the primate lineage with a human-specific loss of copy A. The true orthologous site for copy B may be deactivated in other primates, which would lead an orthology-only scan to incorrectly accept the ORF as having no related ORF in non-human primates).
...
ORFome_filtration.compPrim.badmaps <- ORFome_filtration.summary %>% filter( endsWith(filter, "badmap") ) %>% separate(col=filter, into=c("maptype","genome")) %>% mutate(count = as.numeric(value), genome=measure ) %>% select(-c("value", "measure")) 
...
ORFome_filtration.compPrim.badmaps$genome <- factor(ORFome_filtration.compPrim.badmaps$genome, levels = c("panTro6", "gorGor5", "ponAbe3", "nomLeu3"))
...
ORFome_filtration.compPrim.unseen <- ORFome_filtration.summary %>% filter( endsWith(filter, "unseen") ) %>% separate(col=filter, into=c("maptype","genome")) %>% mutate(count = as.numeric(value), genome=measure ) %>% select(-c("value", "measure")) 
...
ORFome_filtration.compPrim.unseen$genome <- factor(ORFome_filtration.compPrim.unseen$genome, levels = c("panTro6", "gorGor5", "ponAbe3", "nomLeu3"))
...
### Analogous Transcriptome ORF Exclusion

The genomic scan is a reasonable first-pass filter, but could be fooled by eg splicing in the outgroup primate. The raw reads from the Non-Human Primate Reference Transcriptome Resource (NHPRTR) [@Pipes2013 ], for chimpanzee and gorilla. These are strand-aware(?) RNA_Seq libraries from pooled tissues. 

two each assembled from published RNA-Seq data (???Scally et al. 2012???) 

```{r echo=FALSE, message=FALSE, warning=FALSE  }


data_sets.df.NHPRTR.sparse <- data_sets.df %>% filter(subgroups=='NHPRTR') %>%  select(c(name,species,sex))
...
rnaSeq_NHPRTR_counts.gt <- data_sets.df.NHPRTR.sparse  %>% group_by(species) %>% summarise(count=n()) %>% ungroup() %>% gt()  %>% tab_header(title=paste("Table ",tbl_cnt, ". Number of NHPRTR RNA-Seq Samples Available", sep = ""), subtitle="by species") %>% grand_summary_rows(columns = vars(count),fns = list(total = ~sum(.)),formatter = fmt_number, decimals=0) #%>% cols_label(population = " ",)
	
rnaSeq_NHPRTR_counts.gt
...
```
```
These reads were pushed through the QC/Trinity pipeline to yield transcriptome assemblies. 
```

Thusfar intractable... instead using assembled transcriptome files obtained from L. Pipes (personal communication, 19 Jan 2016)


For both the chimp and the gorilla transcriptomes, forward and reverse stranded ORFs longer than 50 nt were called. For each individual, the anomalous ORFs were BLASTed against the chimp and gorilla transcriptomes. Hits were collected and noted. Intersections with a transcriptome ORF were given the same filter as the genomic ORFs (ie, it must overlap 50%+ of the query sequence), with the additional requirement that it be a forward-strand map to a forward-strand ORF (it would be interesting if there was reverse-strand transcription in outgroups which flip direction, and not reason to consider it an exclusion criterion)


If such an intersection was found, the ORF was considered to be transcribed as such in the outgroup primate, and thus marked for exclusion.


```{r echo=FALSE, warning=FALSE, message=FALSE}

ORFome_filtration.outgroupTranscriptomeClean.byPrimate <- read_delim("summaries/ORFs/precandidates/transcriptionally_anomalous/trinity.vs_hg38.blat.compPrim_transcripts.summary", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
names(ORFome_filtration.outgroupTranscriptomeClean.byPrimate ) <- c("indv", "read_algn", "primate", "measure", "value")

ORFome_filtration.outgroupTranscriptomeClean.byPrimate <- rbind(ORFome_filtration.outgroupTranscriptomeClean.byPrimate, ORFome_filtration.init %>% mutate(startingORFcount = total_count) %>% select(-c(filter, sense_count, antisense_count, unique_transcripts, transcript_count,orfBearing_frac, total_count, ref_gen, assembler, ass_algn,orfPerTranscript)) %>% gather(key="measure", value="value", startingORFcount) %>% mutate(primate="chimp") %>% select(names(ORFome_filtration.outgroupTranscriptomeClean.byPrimate )))


ORFome_filtration.outgroupTranscriptomeClean.byPrimate <- rbind(ORFome_filtration.outgroupTranscriptomeClean.byPrimate, ORFome_filtration.init %>% mutate(startingORFcount = total_count) %>% select(-c(filter, sense_count, antisense_count, unique_transcripts, transcript_count,orfBearing_frac, total_count, ref_gen, assembler, ass_algn,orfPerTranscript)) %>% gather(key="measure", value="value", startingORFcount) %>% mutate(primate="gorilla") %>% select(names(ORFome_filtration.outgroupTranscriptomeClean.byPrimate )))
...
ORFome_filtration.outgroupTranscriptomeClean.byPrimate.gg <- ggplot(ORFome_filtration.outgroupTranscriptomeClean.byPrimate) + geom_line(aes(x=measure, y=value, group=paste(indv,primate,read_algn), color=primate)) + geom_point(data = . %>% filter(measure != "startingORFcount"), aes(x=measure, y=value, color=primate)) + geom_point(data = . %>% filter(measure == "startingORFcount"), aes(x=measure, y=value), color="black") + scale_y_log10() + labs(title = paste("Figure ",fig_cnt, ". Transcriptionally Anomalous ORFs\nwith Sequence Similarity Detected in NonHuman Primate Transcriptomes\n(grouped by individual)", sep = ""), y="number of ORFs", x="") + theme_bw() 

ORFome_filtration.outgroupTranscriptomeClean.byPrimate.gg 
# png(height =  500, width = 800, filename = paste("results/figures/fig",fig_cnt,"_ong.png", sep=""))
# thing.gg
# dev.off()
# 
...
ORFome_filtration.cleanOutgroupTranscriptome.byPrimate.retentionRates.gt <- 	ORFome_filtration.outgroupTranscriptomeClean.byPrimate %>% spread(key=measure, value=value) %>% mutate(transcript_detected = transcriptHitFound/startingORFcount, ORF_detected = transcriptOrfFound/startingORFcount, filter_retention = 1-ORF_detected) %>% select(c("read_algn","primate","transcript_detected", "ORF_detected", "filter_retention")) %>% gather(key="measure", value="value", transcript_detected, ORF_detected, filter_retention) %>% group_by(read_algn, primate, measure)  %>%  summarise(minimum = min(value), average=mean(value), median=median(value) , maximum = max(value)) %>% ungroup() %>% group_by(read_algn, primate) %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". ORFs from Anomalous Transcripts With No Corresponding ORF in Non-Human Transcriptome", sep = ""), subtitle="no analogous ORF detected in chimp or gorilla") %>% fmt_percent(columns=vars(maximum,minimum,average,median), decimals=1)  %>% cols_label(measure=" ")

ORFome_filtration.cleanOutgroupTranscriptome.byPrimate.retentionRates.gt
...
ORFome_filtration.outgroupTranscriptomeClean <- ORFome_filtration.summary %>% filter(filter == "outgroupTranscriptomeClean") %>% spread(key=measure, value = value) %>% mutate(outgroupTranscriptomeCleanORFcount = total_count) %>% select(-c(total_count))

ORFome_filtration.outgroupTranscriptomeClean <- inner_join(ORFome_filtration.outgroupTranscriptomeClean, ORFome_filtration.init %>% mutate(startingORFcount = total_count) %>% select(-c(filter, sense_count, antisense_count, unique_transcripts, transcript_count,orfBearing_frac, total_count))  , by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))

ORFome_filtration.outgroupTranscriptomeClean  %<>% mutate(outgroupTranscriptomeClean_frac = outgroupTranscriptomeCleanORFcount/startingORFcount)
...
ORFome_filtration.cleanOutgroupTranscriptome.retentionRates.gt <- ORFome_filtration.outgroupTranscriptomeClean %>% select(read_algn, startingORFcount, outgroupTranscriptomeCleanORFcount, outgroupTranscriptomeClean_frac) %>% gather(key="measure", value="value", -c(read_algn)) %>% mutate(measure=case_when(measure=="outgroupTranscriptomeClean_frac" ~ "percent ORFs human-specific", T ~measure))  %>% group_by(read_algn, measure) %>%  summarise(minimum = min(value), average=mean(value), median=median(value) , maximum = max(value)) %>% ungroup() %>% group_by(read_algn) %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". ORFs from Anomalous Transcripts With No Corresponding ORF in Non-Human Transcriptome", sep = ""), subtitle="no analogous ORF detected in chimp or gorilla") %>% fmt_number(columns=vars(maximum,minimum,average,median), decimals=1, suffixing = T)  %>% fmt_percent(columns=vars(maximum,minimum,average,median), rows = measure=="percent ORFs human-specific", decimals=1, )  %>% cols_label(measure=" ")

ORFome_filtration.cleanOutgroupTranscriptome.retentionRates.gt
...
Anomalous ORFs were then BLASTed against all the assembled transcriptomes. This identified false positives which were among the candidates because they passed filters in at least one individual but also failed them in at least one individual. Fragmentary BLAST hits, in which a non-ORF subsequence was identified in the transcriptomes, were tallied; these can include interesting cases such as transcripts from individuals whose genome contains a variant removing the ORF. 


```{r echo=FALSE, message=FALSE, warning=FALSE  }
populationWideLookback.pre <- read_delim("summaries/ORFs/precandidates/transcriptionally_anomalous/trinity.vs_hg38.blat.mapspliceMulti.lookback.summary",  "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)

names(populationWideLookback.pre) <- c("query","redundant","target","measure","value")

#populationWideLookback.pre %>% filter(target %in% c("all","clean")) %>%  mutate(total=as.numeric(measure)) %>% select(c(query, total))
#populationWideLookback.pre %>% filter(target %in% c("all","clean")) %>%  mutate(total=as.numeric(value)) %>% select(c(query, total))
#populationWideLookback.pre %>% filter(!(target %in% c("all","clean")))


populationWideLookback <- inner_join(populationWideLookback.pre %>% filter(!(target %in% c("all","clean"))),populationWideLookback.pre %>% filter(target %in% c("all","clean")) %>%  mutate(total=as.numeric(measure)) %>% select(c(query, total)),by=c("query"="query")) %>% select(-c("redundant")) %>% mutate(fraction=value/total)


example_indv = "HG00100"



```

Here is an example: the `r (populationWideLookback %>% filter(query==example_indv) %>% group_by(query) %>% summarise(total=unique(total)))$total `  anomalous ORFs derived from the `r example_indv ` transcriptome were BLASTed against every transcriptome in the population, including its own. If no sequence similarity detected, this was noted as "noBlast". If sequence similarity was detected, the transcript(s) containing it were examined. If any such transcript had been flagged as overlapping or accumulating onto a forbidden region, that ORF was noted as a "badScript" and put on a ban list. An ORF which had sequence similarity identified (ie, not noBlast) and was not a badScript, was noted as a "goodScript" and considered further. The ORFs were extracted for the goodScript transcripts (both strands, size threshold > 50 nt) If a BLAST hit overlapped such an ORF (currently any overlap - narrow this down some?), it was considered to be a second transcribed example of the query ORF, and it was called "rediscovered". Otherwise (eg, spurious hits, a transcribed variant in which the ORF is missing) it is called "noORF"


```{r echo=FALSE, message=FALSE, warning=FALSE, eval=T}
#%>% gather(key="type", value = "value", value, fraction)



tbl_cnt <- tbl_cnt + 1

populationWideLookback.exampleSummary.gt <- populationWideLookback %>% filter(query==example_indv & target != example_indv)  %>% group_by(query,measure) %>% summarise( max_val=max(value), med_val= median(value), avg_val = mean(value), min_val = min(value), max_frac=max(fraction), med_frac= median(fraction), avg_frac = mean(fraction), min_frac = min(fraction) ) %>% ungroup() %>% group_by(query) %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". Example Lookback Summary ", sep = ""), subtitle=paste("query individual ( ",example_indv,") excluded as outlier",sep="")) %>% tab_spanner(label="maximum", columns = starts_with("max_"))  %>% tab_spanner(label="median", columns = starts_with("med_")) %>% tab_spanner(label="mean", columns = starts_with("avg_")) %>% tab_spanner(label="minimum", columns = starts_with("min_")) %>% cols_label(max_val = "value", med_val = "value",avg_val = "value",min_val = "value",max_frac = "fraction", med_frac = "fraction",avg_frac = "fraction",min_frac = "fraction", measure=" " ) %>% fmt_percent(ends_with("_frac"), decimals = 1) %>% fmt_number(ends_with("_val"), suffixing = T, drop_trailing_zeros = T, decimals=1)
	
populationWideLookback.exampleSummary.gt

#write(rnaSeq_indiv_demog_counts.gt %>%  as_raw_html(), paste("results/tables/tbl",tbl_cnt,"_rnaSeq_indiv_demog.html", sep=""))


```
When these ORFs were BLASTed against all 1kGen transcriptomes, the fraction for which there was no hit detected at all varied from ~16% to the overwhelming majority (~85%); these are retained by default. ORFs which have sequence similarity detected but never on banned transcripts range from ~12% to almost ~33% , with an overall rejection rate by the lookback filter ranging from ~3% to ~60%. Rediscovery rates ranged higher than noORF rates. 

```{r echo=FALSE, include=TRUE, message=FALSE}
#library("ggalluvial")

fig_cnt <- fig_cnt + 1
populationWideLookback.example.coarse.gg <- ggplot(populationWideLookback %>% filter(query==example_indv, measure %in% c("noBlast","goodScripts","badScripts")) ) + geom_bar(position="stack", stat="identity", aes(x=target, y=value, fill=measure, color=measure))  + geom_bar(data = . %>% select(-c("fraction","total")) %>% spread(key=measure, value=value) %>% mutate(retained=goodScripts+noBlast) %>% select(-c("goodScripts","noBlast", "badScripts")),aes(x=target, y=retained), stat="identity", fill="white", color="black", alpha=0)  + labs(title=paste("Figure ",fig_cnt, "a. Coarse Distribution of Lookback Filter Results\nfor Anomalous ORFs from ", example_indv,  sep = ""), x="target transcriptome searched", y="# ORFs from query")+  scale_x_discrete(breaks=c(example_indv), guide=guide_axis(angle=90))  + theme_bw()

populationWideLookback.example.coarse.gg
...
fig_cnt <- fig_cnt + 1
populationWideLookback.example.fine.gg <- ggplot(populationWideLookback %>% filter(query==example_indv, measure %in% c("noBlast","noORF","rediscovered","badScripts")) ) + geom_bar(position="stack", stat="identity", aes(x=target, y=value, fill=measure, color=measure)) + geom_bar(data = . %>% select(-c("fraction","total")) %>% spread(key=measure, value=value) %>% mutate(retained=noORF+rediscovered+noBlast) %>% select(-c("noBlast", "badScripts", "noORF", "rediscovered")),aes(x=target, y=retained), stat="identity", fill="white", color="black", alpha=0)  + labs(title=paste("Figure ",fig_cnt, "b. Fine Distribution of Lookback Filter Results\nfor Anomalous ORFs from ", example_indv,  sep = ""), x="target transcriptome searched", y="# ORFs from query") +  scale_x_discrete(breaks=c(example_indv), guide=guide_axis(angle=90)) + theme_bw() 





populationWideLookback.example.fine.gg
...
Across all non-self targets, `r example_indv ` anomalous ORFs had a retention rate (ie, were not classified as badScript) of ~ 78%, though taken together it is the minimum of ~ 41% which sets a ceiling on the overall retention rate. 
...
populationWideLookback.exampleSummary.retention.gt <- populationWideLookback %>% filter(query==example_indv & target != example_indv & measure =="badScripts" )  %>% mutate(retention_percent = 1-fraction) %>% group_by(query) %>% summarise( max_frac=max(retention_percent), med_frac= median(retention_percent), avg_frac = mean(retention_percent), min_frac = min(retention_percent) ) %>% ungroup()  %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". Example Lookback Overall Retention ", sep = ""), subtitle=paste("query individual ( ",example_indv,") excluded as outlier",sep="")) %>% tab_spanner(label="maximum", columns = starts_with("max_"))  %>% tab_spanner(label="median", columns = starts_with("med_")) %>% tab_spanner(label="mean", columns = starts_with("avg_")) %>% tab_spanner(label="minimum", columns = starts_with("min_")) %>% cols_label(max_frac = " ", med_frac = " ",avg_frac = " ",min_frac = " " ) %>% fmt_percent(ends_with("_frac"), decimals = 1)
	
populationWideLookback.exampleSummary.retention.gt
...
The lookback check of a query against itself has been omitted from these statistics but is important to examine by itself. For one thing, it's conceivable that an ORF could appear on two transcripts in an individual,  with one touching a forbidden zone. It's worth knowing the false negative rate for the BLAST search (noBlast), and the overall rate at which ORFs will be rediscovered. 
...
populationWideLookback.selfSearch.gt <- populationWideLookback %>% filter(query== target) %>% select(-c(total, value)) %>% spread(key=measure, value=fraction) %>% mutate(retention = 1-badScripts ) %>% gather(key="measure", value="fraction", badScripts:retention)  %>% group_by(measure) %>% summarise( max_frac=max(fraction), med_frac= median(fraction), avg_frac = mean(fraction), min_frac = min(fraction) ) %>% ungroup() %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". Self-Lookback Summary ", sep = ""), subtitle=paste(" ",sep=""))  %>% cols_label(max_frac = "maximum", med_frac = "median",avg_frac = "mean",min_frac = "minimum", measure=" " ) %>% fmt_percent(ends_with("_frac"), decimals = 1) 
	
populationWideLookback.selfSearch.gt
...
ORFome_filtration.lookbackClean <- ORFome_filtration.summary %>% filter(filter=="lookbackClean") %>% spread(key=measure, value = value) %>% mutate(lookbackCleanORFcount = total_count) %>% select(-c(total_count))

ORFome_filtration.lookbackClean <- inner_join(ORFome_filtration.lookbackClean, ORFome_filtration.init %>% mutate(startingORFcount = total_count) %>% select(-c(filter, sense_count, antisense_count, unique_transcripts, transcript_count,orfBearing_frac, total_count))  , by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))

ORFome_filtration.lookbackClean %<>% mutate(lookbackCleanORF_frac = lookbackCleanORFcount/startingORFcount)
...
ORFome_filtration.lookbackClean.retentionRates.gt <- ORFome_filtration.lookbackClean %>% select(read_algn, startingORFcount, lookbackCleanORFcount, lookbackCleanORF_frac) %>% gather(key="measure", value="value", -c(read_algn)) %>% mutate(measure=case_when(measure=="lookbackCleanORF_frac" ~ "percent ORFs w/o lookback badScript", T ~measure))  %>% group_by(read_algn, measure) %>%  summarise(minimum = min(value), average=mean(value), median=median(value) , maximum = max(value)) %>% ungroup() %>% group_by(read_algn) %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". ORFs from Anomalous Transcripts which are Lookback Clean", sep = ""), subtitle="no BLAST hit to a problematic trascript in any 1kGen transrtiptome") %>% fmt_number(columns=vars(maximum,minimum,average,median), decimals=1, suffixing = T)  %>% fmt_percent(columns=vars(maximum,minimum,average,median), rows = measure=="percent ORFs w/o lookback badScript", decimals=1, )  %>% cols_label(measure=" ")

ORFome_filtration.lookbackClean.retentionRates.gt 
...
For each individual transcriptome, a collection of "precandidate" ORFs were called which met the following criteria:

•	No duplicate in the "mainstream" hg38 reference genome.
•	No BLAST hit to anything in the NR PDB
•	No analogous ORF can be found in the reference genome of any other primate examined (chimp, gorilla, gibbon, orangutan)
•	No analogous ORF can be found in the reference transcriptome of any other primate examined (chimp, gorilla)
•	No analogous ORF analogous ORF can be found on on a "forbidden" transcript in any other individual in the 1kGen population


```{r echo=FALSE, warning=FALSE, message=FALSE}


ORFome_filtration.precand <- ORFome_filtration.summary %>% filter(filter=="precandidate") %>% spread(key=measure, value = value) %>% mutate(precandidateORFcount = total_count) %>% select(-c(total_count))

ORFome_filtration.precand <- inner_join(ORFome_filtration.precand, ORFome_filtration.init %>% mutate(startingORFcount = total_count) %>% select(-c(filter, sense_count, antisense_count, unique_transcripts, transcript_count,orfBearing_frac, total_count))  , by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))

ORFome_filtration.precand %<>% mutate(precandidateORF_frac = precandidateORFcount/startingORFcount)

ORFome_filtration <- ORFome_filtration.init %>% mutate(startingORFcount=total_count) %>%  select(c(indv, assembler, ref_gen, ass_algn, read_algn, startingORFcount))
	
ORFome_filtration <- inner_join(ORFome_filtration, ORFome_filtration.nodupe%>%  select(c(indv, assembler, ref_gen, ass_algn, read_algn, noDupORFcount, noDupe_frac)), by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))

ORFome_filtration <- inner_join(ORFome_filtration, ORFome_filtration.noPDB %>%  select(c(indv, assembler, ref_gen, ass_algn, read_algn, noPDBORFcount, noPDB_frac)), by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))


ORFome_filtration <- inner_join(ORFome_filtration, ORFome_filtration.cleanGenome %>%  select(c(indv, assembler, ref_gen, ass_algn, read_algn, cleanGenomeORFcount, cleanGenome_frac)), by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))

ORFome_filtration <- inner_join(ORFome_filtration, ORFome_filtration.outgroupTranscriptomeClean %>%  select(c(indv, assembler, ref_gen, ass_algn, read_algn, outgroupTranscriptomeClean_frac, outgroupTranscriptomeCleanORFcount)), by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))


ORFome_filtration <- inner_join(ORFome_filtration, ORFome_filtration.lookbackClean %>% select(c(indv, assembler, ref_gen, ass_algn, read_algn, lookbackCleanORF_frac, lookbackCleanORFcount)), by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))



ORFome_filtration <- inner_join(ORFome_filtration, ORFome_filtration.precand %>%  select(c(indv, assembler, ref_gen, ass_algn, read_algn, precandidateORF_frac, precandidateORFcount)), by=c("indv"="indv", "assembler"="assembler", "ref_gen"="ref_gen", "ass_algn" = "ass_algn", "read_algn"="read_algn"))
...
ORFome_filtration.gath <- ORFome_filtration  %>% select(c(indv, assembler, ref_gen, ass_algn, read_algn, ends_with("_frac") )) %>% gather(key="measure", value="value", ends_with("_frac"))

ORFome_filtration.gath$measure <- factor(ORFome_filtration.gath$measure, levels = c("noDupe_frac" , "lookbackCleanORF_frac", "outgroupTranscriptomeClean_frac", "noPDB_frac","cleanGenome_frac" ,"precandidateORF_frac"))

ORFome_filtration.retentionRates.gt <- ORFome_filtration.gath %>% group_by(assembler, ref_gen, ass_algn, read_algn,measure)%>%  summarise(minimum = min(value), average=mean(value), median=median(value) , maximum = max(value)) %>% ungroup() %>% group_by(read_algn) %>% gt() %>% tab_header(title=paste("Table ",tbl_cnt, ". Overall Retention Rates at Different Steps of Precandidate Screening", sep = ""), subtitle="percent of anomalous ORFs retained by filter")%>% fmt_percent(columns=vars(maximum,minimum,average,median))  %>% cols_label(measure=" ") %>% cols_hide(c("assembler", "ref_gen","ass_algn"))

ORFome_filtration.retentionRates.gt
...
Across $`r nrow( ORFome_filtration  %>% select(indv) %>% unique() ) %>% human_readable_croncher() `$  individual transcriptomes queried, a total of $`r sum( ORFome_filtration %>% filter(read_algn == "mapspliceMulti") %>% select(precandidateORFcount) ) %>% human_readable_croncher() `$ precandidate ORFs were identified in the mapspliceMulti alignment. This includes  $`r nrow( ORFome_filtration %>% filter(read_algn == "mapspliceMulti") %>% filter(precandidateORFcount ==0) ) %>% human_readable_croncher() `$ individuals in which no such candidates were identified. 

These precandidate ORFs were aggregated into precandidate genes by two different methods: by genomic location and by sequence similarity discovered in individual transcriptomes. 

### Genomic Location Mapping

First, the mapped locations were compared for all precandidate ORFs across all individual transcriptomes sampled. The requirement that ORFs have no duplicates in the genome means that these locii can be easily compared. Pairwise overlaps were merged iteratively to form collections of ORFs representing observed transcription at the same genomic locus. 

### Transcriptome Lookback Rediscovery

Second, the BLAST results from searching individual transcriptomes were reexamined. Specifically, pairwise overlaps of matched query ORFs and transcriptomic ORFs were collected and merged iteratively. Because the query sequences are recognized precandidates, each aggregate contains at least one such precandidate. The rediscovery process has the potential to associate a query ORF with an unrelated ORF in the target sequence, so the ORFs in the resulting categories were subdivided into those whose transcriptomic coordinates are a recognized precandidate, and "hop-ons" which were not for some reason. (the rediscovery process will also drop precandidates from aggregation if they are not rediscovered during self-lookback, but this rarely happens). 

### Compare and Merge the Glomps

These two clustering strategies were compared by examining the pairwise set identity of the clusters generated in each. 

```{r echo=FALSE, include=TRUE, message=FALSE}

precandidates_blat_mapspliceMulti_glomp_by_location <- read_table2("ORFs/precandidates/precandidates.blat.mapspliceMulti.glomp_by_location.list", col_names = FALSE)
names(precandidates_blat_mapspliceMulti_glomp_by_location) <- c("aggregate","instance")

precandidates_blat_mapspliceMulti_glomp_by_rediscovery <- read_table2("ORFs/precandidates/precandidates.blat.mapspliceMulti.glomp_by_rediscovery.list", col_names = FALSE)
names(precandidates_blat_mapspliceMulti_glomp_by_rediscovery) <- c("aggregate","instance")

df.tmp <- as.data.frame(c())
for (seqGlmp in unique(precandidates_blat_mapspliceMulti_glomp_by_rediscovery$aggregate) ) {
	
	precandidates_blat_mapspliceMulti_glomp_by_rediscovery.agg.list <-(precandidates_blat_mapspliceMulti_glomp_by_rediscovery %>% filter(aggregate == seqGlmp) %>% as.list())$instance
	
	
	for (locGlmp in unique(precandidates_blat_mapspliceMulti_glomp_by_location$aggregate) ) {
	
		precandidates_blat_mapspliceMulti_glomp_by_location.agg.list <-(precandidates_blat_mapspliceMulti_glomp_by_location %>% filter(aggregate == locGlmp) %>% as.list())$instance
		num_seq <- length(precandidates_blat_mapspliceMulti_glomp_by_rediscovery.agg.list)
		num_loc <- length(precandidates_blat_mapspliceMulti_glomp_by_location.agg.list)
		insect = length(intersect(precandidates_blat_mapspliceMulti_glomp_by_rediscovery.agg.list,precandidates_blat_mapspliceMulti_glomp_by_location.agg.list))
		iww = length(union(precandidates_blat_mapspliceMulti_glomp_by_rediscovery.agg.list,precandidates_blat_mapspliceMulti_glomp_by_location.agg.list))
		df.tmp <- rbind(df.tmp, c(seqGlmp,locGlmp,num_seq,num_loc,insect, iww))
		

	}
	
}
names(df.tmp) <-c("sequenceGlomp", "locationGlomp", "sequenceGlomp_size", "locationGlomp_size", "intersection", "union" )
precandidates.blat.mapspliceMulti.aggregation.glompComparison <- df.tmp %>% as_tibble() %>% mutate(intersection = as.numeric(intersection) , union = as.numeric(union), similarity = intersection/union)



precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt <- precandidates.blat.mapspliceMulti.aggregation.glompComparison %>% select(ends_with("Glomp") | similarity) %>% spread(key=locationGlomp, value = similarity)


#rownames(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt) <- precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt$sequenceGlomp

rho_names <- precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt$sequenceGlomp

precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt %<>% select(-c("sequenceGlomp")) 


acc <- c()
for (nom in names(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt) ){
	mack = 1
	acc<- c(acc, seq(1,nrow(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt))[(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt %>% select(nom)) ==mack])
	
}

for (j in seq(1,nrow(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt))){
	if (!is.element(j,acc)){
	acc<- c(acc, j)
	}
}

precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt %<>% as.matrix() 

rownames(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt) <- rho_names

precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt<-precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt[acc,]




#precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt[0,]
library("circlize")
precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt <- precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt*100

col_fun = colorRamp2(c( 0, 100), c("white", "black"))
fig_cnt <- fig_cnt + 1

precandidates.blat.mapspliceMulti.aggregation.glompComparison.ht <- Heatmap(precandidates.blat.mapspliceMulti.aggregation.glompComparison.matt, col = col_fun, show_row_dend = F, show_column_dend = F, heatmap_legend_param = list(title = "Similarity %",  border = "black"), column_title = paste("Figure ", fig_cnt , ". Consolidating the Precandidates:\nshared groupings under 2 glomping strategies", sep="") )
precandidates.blat.mapspliceMulti.aggregation.glompComparison.ht

```

sequenceGlomp_0 refers to a single ORF sighting which doesn't map well to hg38. The locationGlomp precandidates with no overlap to the sequenceGlomps are often repetitive and don't blast well to other transcriptomes, including the source transcriptome.

So there are a few cases where sequences are being retained partly because they don't align easily. However, where they overlap the two aggregation strategies agree completely. 

For now, we'll concentrate on the cases which are both easily localized in the genome and easily detected in the transcriptome (ie, a simple inner-join).


### Unscreened Precandidates

```{r echo=FALSE, include=TRUE, message=FALSE}
precandidates_basicFacts <- read_delim("summaries/precandidates.basicFacts.summary", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
names(precandidates_basicFacts) <- c("qual","read_aligner", "measure", "value")
...
precandidates_basicFacts.gt <- precandidates_basicFacts %>% spread(key=measure, value=value)%>% group_by(qual) %>%  gt()  %>% tab_header(title=paste("Table ",tbl_cnt, ". Precandidate Merging Process", sep = ""), subtitle="precandidate glomps retained/excluded") %>% tab_spanner(label = "excluded from merge", columns = c("distillationGhosts_glompcount","distillationGhosts_ORFcount")) %>% cols_label(distillationGhosts_glompcount = "# glomps", distillationGhosts_ORFcount = "# ORFs", expresser_count	= "# ORF expressers", locusCount = "# gene models", orf_count = "# examples", precandidate_count = "# precandidates", read_aligner = " ")


precandidates_basicFacts.gt
...
This joining process merged 10 ORFs from 4 expressing individuals into 7 precandidates for further examination.  15 previously identified precandidate ORFs were excluded on the basis of unreliable mappability. Each precandidate had a single gene model associated.


eval vvv

```{r echo=FALSE, warning=FALSE, message=FALSE, eval=F}
precandidates_mapspliceMulti <- read_delim("precandidates/unscreened/mapspliceMulti/precandidates.mapspliceMulti.roster", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
names(precandidates_mapspliceMulti) <- c("glomp","example","precandidate")

#

#precandidates_mapspliceMulti %>% mutate(stage="unscreened", read_algn = "mapspliceMulti") %>% separate(col=example, into=c("individual", "ORF"), sep = ";") %>% select(-c("glomp"),"precandidate", "individual", "ORF") %>% gt()

tbl_cnt <- tbl_cnt + 1

precandidates_mapspliceMulti.gt <- precandidates_mapspliceMulti  %>% separate(col=example, into=c("individual", "ORF"), sep = ";") %>% select(-c("glomp"),"precandidate", "individual", "ORF") %>% group_by(precandidate) %>% gt()  %>% tab_header(title=paste("Table ",tbl_cnt, ". Precandidates Called", sep = ""), subtitle="unscreened, mapspliceMulti alignment")%>% cols_label( precandidate = " ")


precandidates_mapspliceMulti.gt
...
How frequently are the precandidates transcribed? First, here are the number of individual ORF sightings from which the precandidate clusters were derived. Since each precandidate comes from at least one sighting, this distribution has a floor of 1. 

```{r echo=FALSE, warning=FALSE, message=FALSE}

unscreened_expressorsPerPre <- read_delim("summaries/precandidates/unscreened.expressorsPerPre.hist", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
names(unscreened_expressorsPerPre) <- c("read_algn", "null", "count", "precandidate")
unscreened_expressorsPerPre %<>% select(-c("null")) 


hist_size <- max(unscreened_expressorsPerPre$count)

fig_cnt <- fig_cnt + 1
unscreened_expressorsPerPre.gg <- ggplot(unscreened_expressorsPerPre ) + geom_histogram(breaks=seq(0.5,hist_size+0.5,1), aes(x=count)) + labs(title=paste("Figure ",fig_cnt, "a. Detection Frequency Histogram for Precandidates:\nSource ORFs per Precandidate",  sep = ""), x="# individuals with ORF in transcriptome", y="# precandidates") + theme_bw() 

unscreened_expressorsPerPre.gg
...
The precandidates also had instances of sequence similarity identified in the queried transcriptomes, but which were not open reading frames. Upon closer examination, these appeared to be truncated transcripts in which the ORF sequence was clipped. Including these:


```{r echo=FALSE, warning=FALSE, message=FALSE}


unscreened_nonORFHits_PerPre <- read_delim("summaries/precandidates/unscreened.nonORFHits_PerPre.hist", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)

names(unscreened_nonORFHits_PerPre) <- c("read_algn", "precandidate","null", "count")
#unscreened_nonORFHits_PerPre %<>% select(-c("null")) 


unscreened_allXprsrs_PerPre <- rbind(unscreened_expressorsPerPre %>% mutate(type = "ORF"),unscreened_nonORFHits_PerPre %>% select("read_algn", "count" ,"precandidate") %>% mutate(type="nonORF"))

unscreened_allXprsrs_PerPre <- rbind(unscreened_allXprsrs_PerPre, unscreened_allXprsrs_PerPre %>% group_by(precandidate, read_algn) %>% summarise(count=sum(count), type="total"))



hist_size <- max(unscreened_allXprsrs_PerPre$count)

#fig_cnt <- fig_cnt + 1
unscreened_allXprsrs_PerPre.gg <- ggplot(unscreened_allXprsrs_PerPre ) + geom_histogram(breaks=seq(0.5,hist_size+0.5,1), aes(x=count,fill=type), position = "stack") + labs(title=paste("Figure ",fig_cnt, "b. Detection Frequency Histogram for Precandidates:\nAll Sequence Similarity Hits per Precandidate",  sep = ""), x="# individuals with non-ORF sequence similarity in transcriptome", y="# precandidates") + theme_bw() 

unscreened_allXprsrs_PerPre.gg
...
We can also ask, how many precandidates are sighted per transcriptome examined? In most cases, none:


```{r echo=FALSE, warning=FALSE, message=FALSE}

unscreened_prePerExpressor <- read_delim("summaries/precandidates/unscreened.prePerExpressor.hist", "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
names(unscreened_prePerExpressor) <- c("read_algn", "null", "count", "individual")
unscreened_prePerExpressor %<>% select(-c("null")) 


unscreened_prePerIndv <- full_join(data_sets.df.sparse %>% select(name), unscreened_prePerExpressor, by = c("name"="individual") ) %>% mutate(count=case_when(is.na(count)~0, T~count))




hist_size <- max(unscreened_prePerIndv$count)

fig_cnt <- fig_cnt + 1
unscreened_prePerIndv.gg <- ggplot(unscreened_prePerIndv ) + geom_histogram(breaks=seq(-0.5,hist_size+0.5,1), aes(x=count)) + labs(title=paste("Figure ",fig_cnt, "a. Number of Precandidates Detected per Transcriptome",  sep = ""), x="# precandidates per individual", y="# individuals") + theme_bw() 

unscreened_prePerIndv.gg
...
# 
```
(go back and gather the # of individual with non-ORF transcriptome hits??)

Number of precandidates per transcriptome examined... roll in the partial results as above
...
# Procedure: Followup Data

## Control Transcripts & ORFs

To collect representative non-genic ORFs, the annotations used to exclude mapped transcripts were broken up into exons; these were slopped by the same amount (75 bp) and then subtracted from the mainstream chromosomes of the hg38 reference genome. These remaining intervals were downsampled by selecting 2500 at random, and from the sequence in these intervals ORFs were called using both strands and the same length cutoff as with the 1kGen transcriptome (75bp). These ORFs are referred to as "non-exonic".

This process was repeated using the annotation locus as the interval of exclusion rather than the exon; this is to say, no ORFs are called from intronic regions in this control set. These ORFs are referred to as "intergenic".
...
One interesting question about a given peptide is: how typical are its properties, given its composition? This can be investigated by scrambling its sequence. One approach is to shuffle the codons in between the start and stop; the other is to shuffle the underlying nucleotides. These measure slightly different things: the former can test for interesting structural interactions between AAs which are broken up by shuffling while the latter can test for unusual codon usage given a GC%.

Shuffling nucleotides has the added complexity that it can generate new stop codons within the sequence, and simple reshuffling is prohibitively inefficient at finding unobstructed ORFs for realistic sequence lengths. To get around this, an interative reshuffling was applied: if a stop codon interrupts the generated sequence, the sequence from the first such stop on is permuted, popping the first nucleotide from the sequence and appended to the end, thus necessarily removing that first stop while preserving length and content. The process is then applied on the altered sequence until no stops remain.

25 scrambled replicates were generated for each ORF in each category (candidate, intergenic, nonexonic, mRNA)
...
recap the ORF discovery process & compare/contrast with the control ORFs..
