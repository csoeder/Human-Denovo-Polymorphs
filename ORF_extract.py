...
A script to pull ORFs out of transcripts
	usage:
	python ORF_extract transcripts_IN.fa ORFs_OUT.fa
'''
...
def extractor(record):
	...
	start = re.compile(r'ATG')  # start codon, aka Methionine.
	# 							Alternatives rare in eukaryotes. Source: Wikipedia
	stop = re.compile(r'T(?!GG)[AG][AG]')
	# Scan the given seqeunce for instances of codon,
	# tags their locations in a dictionary dic
	...
	def find_ORFs(start_dict, stop_dict, mod):
		...
		while last < len(seek):
			try:
				...
				#######################################################
				# This is different. ORFs haven't been aligned yetself.
				# chro = desc.split('\t')[0][1:]#grab the chromosome name
				# a, b = desc.split('\t')[1:3]#grab the coords
				# This will instead provide coordinates
				# relative to the transcriptome assembly...
