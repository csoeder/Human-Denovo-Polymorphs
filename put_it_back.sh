...
for d00d in $( grep $subsetFilter d00ds.list ); do
	...
	scp -o ControlPath="$SSHSOCKET" individuals/"$d00d"/Trinity_files.Trinity.fasta* "$USER_AT_HOST":/netscr/csoeder/1kGen/individuals/"$d00d"/  ;
	scp -o ControlPath="$SSHSOCKET" individuals/"$d00d"/"$d00d"_blatAlign.goodAlign.ILS_anomalies.bed.derpy "$USER_AT_HOST":/netscr/csoeder/1kGen/individuals/"$d00d"/ ;
	scp -o ControlPath="$SSHSOCKET" individuals/"$d00d"/ILS_*.list "$USER_AT_HOST":/netscr/csoeder/1kGen/individuals/"$d00d"/ ;
	scp -o ControlPath="$SSHSOCKET" individuals/"$d00d"/mapt/* "$USER_AT_HOST":/netscr/csoeder/1kGen/individuals/"$d00d"/mapt/ ;
done ;
