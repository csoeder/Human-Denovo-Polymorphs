...
curr.execute("SELECT location_pk FROM location;")
...
for jean in all_genes:
	...
	curr.execute('SELECT person_pk FROM person WHERE person.person_pk NOT IN (SELECT source FROM find WHERE find.loc = %s);', tuple([jean[0]]))
