...
#	!!!!!!!!!!!!!!!!! ^^^^ formally put this in the pipeline!
...
#	grep ">" individuals/"$d00d"/mapt/precleaned_ORFs.fa | tr -d ">" | sed -e 's/:/xy/' | tr 'y' '|' | tr '-' '|' > precleaned_ORFs.predat.tmp ;
	grep ">" individuals/"$d00d"/mapt/precleaned_ORFs.fa | tr -d ">" | tr ':' '|' | tr '-' '|' > precleaned_ORFs.dat.tmp ;
...
#	echo "CREATE TABLE temp (trans int, start int, stop int );" | sqlite3 $db_file ;
	echo "CREATE TABLE temp (comp_name varchar(50), start int, stop int );" | sqlite3 $db_file ;
...
#	echo "INSERT INTO orf (trans, start, stop) SELECT * FROM temp;" | sqlite3 $db_file ;
	time echo "INSERT INTO orf (trans, start, stop) SELECT s.trans_pk, t.start, t.stop FROM temp t, transcript s WHERE t.comp_name=s.comp_name;"| sqlite3 $db_file ;
	echo "DROP TABLE temp;" | sqlite3 $db_file ;
...
for i in $( cut -f 1 lookback.pretable | sort | uniq ); do
	awk '{if($8==1 && $9==$10)print;}' lookback_"$i".dat > full_lookback_"$i".dat ;
...
while read bedline; do 
	...
	for d00d in $( echo "select person_name from person where person.person_pk in (select source from transcript where transcript.trans_pk in (select trans from alignment where alignment.aln_pk in (select aln_id from precandidate_lookup where precand="$precand_pk")));" | sqlite3 $db_file | sort | uniq ) ; do 
		bedtools intersect -wa -u -a bedline.bed.tmp -b individuals/"$d00d"/"$d00d"_splicedReads.introns.bed | sed -e 's/^/'$d00d'\t/' | cut -f 1 | sort | uniq >> precandidate_splicingEvidence.list ; 
	done;
...
done; < precandidate_locations.bed4
