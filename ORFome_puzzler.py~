...
slop = 100 #	how many nt around the alignment do we look???
frac_pept = 0.75 	#	how much of the sequence must be in an ORF???

def forward_ORFome():
	seq_query = check_output(['samtools', 'faidx', COMP_GENOME, '%s:%s-%s'%tuple([chrom, start-slop, stop+slop])])
	seek = Seq.Seq(''.join(seq_query.split('\n')[1:]).upper(), Bio.Alphabet.generic_dna).tostring()
	forward_orfs = find_ORFs_forward(find_starts_forward(seek), find_stops_forward(seek))
	phial = open('ORFome.tmp','w')
	for orf in forward_orfs:
		phial.write( "%s\t%s\t%s\n"%tuple([chrom, start-slop+orf[0]-1, start-slop+orf[1]+2]))
	phial.close()

def reverse_ORFome():
	seq_query = check_output(['samtools', 'faidx', COMP_GENOME, '%s:%s-%s'%tuple([chrom, start-slop, stop+slop])])
	seek = Seq.Seq(''.join(seq_query.split('\n')[1:]).upper(), Bio.Alphabet.generic_dna).tostring()
	reverse_orfs = find_ORFs_reverse(find_starts_reverse(seek), find_stops_reverse(seek))
	phial = open('ORFome.tmp','w')
	for orf in reverse_orfs:
		phial.write("%s\t%s\t%s\n"%tuple([chrom, start-slop+orf[1]-1,  start-slop+orf[0]+2,]))
	phial.close()
...
# bedtools intersect  -f 0.5 -b ORFome.tmp -a test.bed 