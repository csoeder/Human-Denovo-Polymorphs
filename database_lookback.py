...
os.system('while [[ `bjobs -w | grep retromap_ | wc -l` -gt 0 ]]; do sleep 60; bar=$(); f="#"; percent=$((100*$(bjobs -w | grep retromap_ | wc -l)/%s)) ; for i in {1..$((50*$(bjobs -w | grep retromap_ | wc -l)/%s))}; done; do bar=$bar$f; echo -ne "$percent%% done $bar" ; done'%tuple([len(all_peeps), len(all_peeps)]) )
