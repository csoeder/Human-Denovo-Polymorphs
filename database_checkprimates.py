...
primate_omes = '/lustre/scr/c/s/csoeder/primates/'
...
for sample in check_output('ls %s | grep -v the'%primate_omes, shell=True).split('\n'):
...
for sample in check_output('ls %s | grep -v the | grep pan'%primate_omes, shell=True).split('\n'):
	chumps = set(np.array(check_output('cut -f 4 %s/%s_lookback.bed | sort | uniq'%tuple([primate_omes, sample]), shell=True).split('\n')))
for sample in check_output('ls %s | grep -v the | grep gor'%primate_omes, shell=True).split('\n'):
