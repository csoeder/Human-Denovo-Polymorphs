...
curr.execute("CREATE TABLE location ( location_pk serial PRIMARY KEY, chrom varchar(20), start bigint, stop bigint, ref varchar(20), var text, poly boolean, canon_seq references sequence(sequence_pk), handchecked boolean, meta text ) ;")
...
curr.execute("CREATE TABLE chimp_location ( panLoc_pk serial PRIMARY KEY, chrom varchar(20), start bigint, stop bigint, ref varchar(20), sequence references sequence(sequence_pk), pan145_cov float(10), pan146_cov float(10), meta text ) ;")
...
curr.execute("CREATE TABLE gorilla_location ( gorLoc_pk serial PRIMARY KEY, chrom varchar(20), start bigint, stop bigint, ref varchar(20), sequence references sequence(sequence_pk), gor142_cov float(10), gor143_cov float(10), meta text ) ;")
# Define a genomic location for the homologous sequences in chimps
#	pk, chromosome, start, stop, reference genome, sequence, mapsplice coverage per gor142; per gor143; 

curr.execute("CREATE TABLE find (find_pk serial PRIMARY KEY, source serial references person(person_pk), seq serial references sequence(sequence_pk), loc serial references location(id), assembly text, meta text )")
