...
#	pyVCF's heterozygosity method is flaky; here's one that doesn't fail all over the place
def hetZyggy(wreck):	
	gt_dict={}
...
for rec in parser:
	...
	for het in rec.get_hets():
		lookup_sql.write('%s\t%s\t%s\t\n'%tuple([variant_pk,het.sample,'%s|%s'%tuple(het.gt_alleles)]))
	...
	#variant_sql.write("INSERT INTO variant (variant_pk, chrom, pos, ref_allele, alt_alleles, heterozygosity, varType, varSubtype, pi_hat, snpDBid) VALUES (%s,%s, %s,%s,%s,%s,%s,%s,%s,%s);", tuple([variant_pk,"chr%s"%rec.CHROM, rec.POS, rec.REF, hets, hetZyggy(rec), rec.var_type, rec.var_subtype, piHat(rec), rec.ID])
