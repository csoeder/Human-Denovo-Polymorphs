...
# header:	#Status	individuals/HG00096/HG00096_mapsplice_alignment.sort.bam\t
# individuals/HG00097/HG00097_mapsplice_alignment.sort.bam	individuals/HG00...
with open(feature_count_summary) as phial:
	reader = csv.reader(phial, delimiter="\t")
	rawheader = reader.next()
	rows = list(reader)

# make an ordered list of the individuals
header = [i.split("/")[1] for i in rawheader[1:]]

# package the full-scale alignment stats
summary_dict = {}
for row in rows:
	summary_dict[row[0]] = [float(i) for i in row[1:]]

unassigned_mapped_reads = np.array([
	float(i) for i in summary_dict['Unassigned_NoFeatures']])
assigned_mapped_reads = np.array([float(i) for i in summary_dict['Assigned']])
# total read count for FPKM calc
total_mapped_reads = list(unassigned_mapped_reads + assigned_mapped_reads)

# open and parse the roster of RNA-Sequenced individuals.
# Status	individuals/HG00096/HG00096_mapspli e_alignment.sort.bam
# ...individuals/HG00097/HG00097_mapsplice_alignment.sort.bam ...
with open(feature_count_data) as phial:
	reader = csv.reader(phial, delimiter="\t")
	reader.next()
	reader.next()
