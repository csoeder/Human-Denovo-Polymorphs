...
polymorphic_denovo_SQL="FROM candidate WHERE poly=1 and charlie_thumbs='up'"
...
echo "Chromosome X:"
echo "SELECT chrom chromosome, count(*) number_genes $polymorphic_denovo_SQL and chrom='chrX';" | mysql --user=csoeder --password=tha_snazzle glurge -N
...
echo "SELECT l.precand_id, t.coding_prob, t.orf_len, t.orf_len/t.mrna_len, t.fickett_score, t.hexamer_score FROM transcript t, candidate c, precandidate_lookup l, orf o WHERE o.trans=t.comp_name AND o.person_name=t.person_name AND o.old_pk=l.orf_id AND o.person_name = l.person_name AND c.precand_pk IN (SELECT precand_pk $polymorphic_denovo_SQL ) and l.precand_id=c.precand_pk GROUP BY l.precand_id;" | mysql --user=csoeder --password=tha_snazzle glurge -N > candidates.CPAT.dat
